...
public class Problem_11 {
	...
	public static long solution_0(int row, int col) {
		long highest = 0;
		int[][] matrix;

		try {
			matrix = EulerUtils.readMatrix("20x20.txt", row, col);
		...
		for (int i = 0; i < row; i++)
			for (int j = 0; j < col - 3; j++) {
				if (matrix[i][j] * matrix[i][j + 1] * matrix[i][j + 2]
						* matrix[i][j + 3] > highest)
					highest = matrix[i][j] * matrix[i][j + 1]
							* matrix[i][j + 2] * matrix[i][j + 3];
				if (matrix[j][i] * matrix[j + 1][i] * matrix[j + 2][i]
						* matrix[j + 3][i] > highest)
					highest = matrix[j][i] * matrix[j + 1][i]
		...
		for (int i = 0; i < row - 3; i++)
			for (int j = 0; j < col - 3; j++) {
				if (matrix[i][j] * matrix[i + 1][j + 1] * matrix[i + 2][j + 2]
						* matrix[i + 3][j + 3] > highest)
					
					highest = matrix[i][j] * matrix[i + 1][j + 1]
							* matrix[i + 2][j + 2] * matrix[i + 3][j + 3];
				
				if (matrix[row - 1 - i][j] * matrix[row - 1 - i - 1][j + 1]
						* matrix[row - 1 - i - 2][j + 2] * matrix[row - 1 - i - 3][j + 3] > highest)
					
					highest = matrix[row - 1 - i][j] * matrix[row - 1 - i - 1][j + 1]
							* matrix[row - 1 - i - 2][j + 2]
							* matrix[row - 1 - i - 3][j + 3];
			}

		return highest;
	}

	public static long solution_1(int row, int col) {
		int[][] A = {
				...
				if (i <= 16) {
					...
					if (j <= 16) {
						D1 = A[i][j] * A[i + 1][j + 1] * A[i + 2][j + 2]
								* A[i + 3][j + 3];
						D2 = A[i][19 - j] * A[i + 1][18 - j] * A[i + 2][17 - j]
								* A[i + 3][16 - j];
					}
				}
				if (j <= 16)
		...
		// cout << "The greatest product of four numbers in a straight line is "
		// << M << ". It begins at row " << B[0] << ", column " << B[1] <<
		// " and goes in direction " << D << "\n";

		return M;
	}

	// /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	public static long solution_2(int row, int col) {
		char[][] grid = {
				...
				{ 19, 80, 81, 68, 5, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17,
						77, 4, 89, 55, 40 },
				{ 04, 52, 8, 83, 97, 35, 99, 16, 7, 97, 57, 32, 16, 26, 26, 79,
						33, 27, 98, 66 },
		...
		for (i = 0; i < 20; i++) {
			for (j = 0; j < 16; j++) {
				prod = grid[i][j] * grid[i][j + 1] * grid[i][j + 2]
						* grid[i][j + 3];
				if (prod > big)
					big = prod;
			}
	...
	// /////////////////////////////////////////////////////////////////////////////////////////////
	public static long solution_3(int row, int col) {
		int[][] array = {
				{ 8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50,
						77, 91, 8 },
				{ 49, 49, 99, 40, 17, 81, 18, 57, 60, 87, 17, 40, 98, 43, 69,
